...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - GENESIS - %(levelname)s - %(message)s')

# Định nghĩa nội dung chuẩn cho TẤT CẢ các tệp cốt lõi
# Đây là phiên bản đã được rà soát và sửa lỗi toàn diện
CORE_FILES_CONTENT = {
    "myiu/app_context.py": """
class AppContext:
    def __init__(self):
...
def setup_logger():
    ...
    if not logger.hasHandlers():
        ...
        file_handler = logging.FileHandler("myiu_system.log")
...
# Dọn dẹp, phiên bản này không cần import gì thêm ở đây
class Affect(AsyncModule):
    def __init__(self, app_context):
        super().__init__()
        self.log.info("Affect module initialized (placeholder).")
    async def start(self):
        await super().start()
    async def stop(self):
        await super().stop()
...
class Memory(AsyncModule):
    def __init__(self, app_context):
        super().__init__()
        self.log.info("Memory module initialized (placeholder).")
    async def start(self):
        await super().start()
    async def stop(self):
        await super().stop()
...
class Cortex(AsyncModule):
    def __init__(self, app_context):
        super().__init__()
        self.log.info("Cortex module initialized (placeholder).")
    async def start(self):
        await super().start()
    async def stop(self):
        await super().stop()
...
def apply_all_patches():
    logging.info("Bắt đầu Kế Hoạch Tái Sinh: Ghi đè toàn bộ lõi hệ thống.")
    for file_path, new_content in CORE_FILES_CONTENT.items():
        try:
            ...
            logging.info(f"TÁI SINH THÀNH CÔNG: {file_path}")
        except Exception as e:
            logging.error(f"TÁI SINH THẤT BẠI cho {file_path}: {e}")
    logging.info("Hoàn tất Kế Hoạch Tái Sinh.")
